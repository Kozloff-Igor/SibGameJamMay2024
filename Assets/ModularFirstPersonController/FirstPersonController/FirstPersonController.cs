...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        artificialGravity = FindObjectOfType<ArtificialGravity>();
        StickyGuy = artificialGravity.transform;
    }
    ...
    private void Update()
    {
        ...
        if(cameraCanMove)
        {
            ...
            float horRot = Input.GetAxis("Mouse X") * mouseSensitivity;
            ...
            transform.Rotate(0, horRot,0, Space.Self);
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            {
                isWalking = false;
                targetVelocity *= 0.1f; //no controllable flying
            }
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                Vector3 proj = Vector3.Project(velocityChange, transform.up);
                velocityChange -= proj;
                //velocityChange.y = 0;
                ...
                //Vector3 velChangePlayerSpace = new Vector3(transform.right * velocityChange.x, transform.up)
    ...
    {
        Vector3 origin = new Vector3(transform.position.x, transform.position.y , transform.position.z) - transform.up * (transform.localScale.y * .5f);
        Vector3 direction = transform.TransformDirection(-Vector3.up);
        //float distance = .75f; //-that was original
        float distance = 1f;

        bool rayDown = Physics.Raycast(origin, direction, out RaycastHit hit, distance);

        bool rayToClosest = Physics.Raycast(origin, (artificialGravity.closestAsteroid.position - transform.position).normalized, distance);

        //if (Physics.Raycast(origin, direction, out RaycastHit hit, distance))
        if (rayDown || rayToClosest)
        //if (Physics.SphereCast(origin, 0.6f, direction, out RaycastHit hit, distance * 2))
        {
